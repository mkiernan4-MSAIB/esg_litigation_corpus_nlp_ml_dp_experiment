...
- `snapshot_07_longformer_v1_4232026.zip` → `6aacfc06bbb0a4debb94baf17c102913bf42659eb5122ed71cf03a90e72bee0f` (1.3 MB — embeddings.npy + labels.npy + snapshot + manifest; checkpoints hashed but excluded)
...
**Reproducibility package (updated April 25, 2026):** `snapshots/` — 7 versioned snapshot CSV+JSON pairs. `step_zips/` — 8 per-step zips (01–07 + 10) each containing `reproducibility_manifest_v1_4232026.json` with SHA-256 for every file inside. Step-06 zip regenerated April 23, 2026 for revised 27-plot outputs (SHA-256: `b7b3e40e...`, 23.6 MB). Step-07 zip added April 25, 2026 (SHA-256: `6aacfc06...`, 1.3 MB — embeddings + labels; checkpoints hashed but excluded due to size). Jupyter notebook integrity check cell asserts frozen matrix hash on load. 5 commits pushed to GitHub via SSH. Remote: `git@github.com:mkiernan4-MSAIB/esg_litigation_corpus_nlp_ml_dp_experiment.git`. No PATs -- SSH ed25519 key at `~/.ssh/id_ed25519`.

**Longformer training results (April 23–24, 2026 — COMPLETE):** `07_esg_longformer.py` executed in Colab T4 GPU. Config: SEEDS=[42,123,7], N_FOLDS=3, EPOCHS=5, BATCH_SIZE=1, GRAD_ACCUM=16, LR=2e-5, FREEZE_LAYERS=8, USE_AMP=True (AMP FP16 + gradient checkpointing). 9/9 folds complete. **Final Longformer Macro-F1: 0.4355 ± 0.0370** across all folds. Seed 7 results: 0.4628 ± 0.0202 [F1 per fold: 0.4625, 0.4381, 0.4877]. CLS embeddings: shape=(444, 768), saved to `esg_corpus_outputs/longformer/longformer_embeddings.npy`. `07_manifest.json` written. Longformer F1=0.4355 vs XGBoost F1=0.7684 — expected for 148M-parameter model on 444-case corpus; primary limitations finding. Reproducibility zip: `snapshot_07_longformer_v1_4232026.zip` (1.3 MB) in `step_zips/` — April 25, 2026. Checkpoints (568 MB × 9) excluded from zip; SHA-256 hashes recorded in reproducibility manifest. Script 08 (`08_esg_xai_visualizations.py`) is now unblocked.

**XAI visualizations (April 25, 2026 — partial):** `08_esg_xai_visualizations.py` executed locally. 4/5 sections complete: `roc_curves_ml_baseline.png`, `word_clouds_by_class.png`, `shap_longformer_partition.png`, `tsne_longformer_embeddings.png` in `esg_corpus_outputs/visualizations/`. Section 5 (attention heatmap) deferred to Colab — requires `torch` not installed locally. `08_manifest.json` written. Script bugs fixed: versioned filename, t-SNE label encoding (le.transform not enumerate index), SHAP class_names from le.classes_, em-dash→'--', `max_iter` not deprecated `n_iter`.
